...
public class SalaryController extends BaseController {
    ...
    private String list(@PathVariable int page) {
        //getSession().setAttribute("pagination", salaryService.list(page));//涉及不到关联查询。
        getSession().setAttribute("pagination", salaryService.query(page, "queryUserSalaries", null));
        getSession().setAttribute("users", userService.queryAll("queryAllUser", null));//add.jsp 中的下拉列表
